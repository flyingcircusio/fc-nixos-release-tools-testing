...
from update_nixpkgs import NIXPKGS_REPO, FC_NIXOS_REPO
...
def rebase_nixpkgs(
    ...
    logging.info(f"Trying to rebase nixpkgs repository.")
...
def update_fc_nixos(
    ...
    logging.info(f"Update fc-nixos.")
...
def create_fc_nixos_pr(
    ...
    logging.info(f"Create PR in fc-nixos.")
...
def run(platform_versions: list[str], nixpkgs_upstream_url: str, nixpkgs_origin_url: str, fc_nixos_dir: str, nixpkgs_dir: str, github_access_token:str):
    ...
    for platform_version in platform_versions:
        ...
        remotes = {
            "upstream": Remote(
                nixpkgs_upstream_url, [nixpkgs_target_branch]
            ),
            "origin": Remote(
                nixpkgs_origin_url,
        ...
        if result := rebase_nixpkgs(
            ...
            logging.info(f"Updated 'nixpkgs' to '{result.fork_after_rebase.hexsha}'")
            update_fc_nixos(
                fc_nixos_dir,
